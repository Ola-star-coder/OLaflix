...
const homePageSections = [
  ...
  },
  {
     ...
     */
    const genreList = {
        //create genre string from genre_id eg: [23,44] -> "action", "romance".
...
fetchDataFromServer(`https://api.themoviedb.org/3/movie/popular?language=en-US&page=1&api_key=${api_key}`, heroBanner);
...
const heroBanner = function({ results: movieList }){
  ...
  for (const [index, movie] of movieList.entries()){
    ...
    sliderItem.innerHTML = `
     ...
     <div class="meta-list">
      ...
      <div class="meta-item card-badge">${vote_average.
       toFixed(1)}</div>
     </div>
  ...
  // fetch data for homepage(top rated,upcoming, trending)

  for (const {title,path} of homePageSections){

    fetchDataFromServer(`
      https://api.themoviedb.org/3${path}?api_key=${api_key}&page=1`, createMovieList, title);
